...
def split(img_name, num):
    ...
    for i in range(num):
        cv2.imwrite('./data/img{}.jpg'.format(i), img[62 * i: 62 * (i + 1) - 2, :])


def run(path):
    pages = convert_from_path(path, 500)
    pages[0].save('./ImgProcess/out.jpg', 'JPEG')

    img = cv2.imread("./ImgProcess/out.jpg", 0)
    table = get_table(img)
    cv2.imwrite('./ImgProcess/table.jpg', table)
    k = table[:, 1658:1743]
    cv2.imwrite("./ImgProcess/digit.jpg", k)
    index_student = ExtractId.pdfparser()
    split('./ImgProcess/digit.jpg', len(index_student))
